...
export default class extends React.Component {
  ...
  genderString() {
    if (this.props.filters.gender.length < 1) {
  ...
  languagesString() {
    ...
    return (
      <span>
        across{" "}
        <span className="stat">
          {numeral(languageCount).format("0,0")}{" "}
  ...
  programString() {
    ...
    return (
      <span>
        by{" "}
        <span className="stat">
          {numeral(affiliateCount).format("0,0")}{" "}
  ...
  render() {
    ...
    return (
      <div className="IntroProse">
        This data dashboard visualizes &nbsp;
        <span className="stat">
          {numeral(this.props.items.length).format("0,0")} testimonies
        </span>
        &nbsp;{this.genderString()}
        &nbsp;born{" "}
        {this.yearRangeString(
          this.props.filters.dateRanges.birth || [
            this.props.BIRTH_MIN,
            this.props.BIRTH_MAX,
          ],
        )}
        &nbsp;{this.birthplaceString()}
        &nbsp;who were interviewed{" "}
        {this.yearRangeString(
          this.props.filters.dateRanges.recording || [
            this.props.RECORDING_MIN,
            this.props.RECORDING_MAX,
          ],
        )}
