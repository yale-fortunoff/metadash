...
export default class extends React.Component {
  ...
  trackScrolling() {
    ...
    const scrollTop = this.poolRef.current.scrollTop,
      scrollBottom =
        scrollTop + this.poolRef.current.getBoundingClientRect().height,
  ...
  render() {
    ...
    return (
      <div ref={this.poolRef} className="count-list">
        {(items || [])
          ...
          .map((item, i) => {
            ...
            if (item.id in this.props.itemDict) {
              className = "list-item";
              itemCount = numeral(this.props.itemDict[item.id].count).format(
                "0,0",
              );
